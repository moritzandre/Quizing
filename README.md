...
| Video           | Plays a YouTube clip with custom controls — the **title stays hidden** (it would give the answer away). Optional **audio-only** mode. The clip **auto-pauses when a player buzzes in**.                                                                                               |
| Clip Ladder     | A video/audio sibling of the Hint Ladder: the host plays a **short slice** of the clip and **extends it step by step** (the **Extend clip** button, the **H** key, or the host remote) — the longer it runs, the fewer points it's worth. Also auto-pauses on buzz.                  |
...
- **Host remote** — open `#/host/<code>` on a phone to drive the whole game (reveal, advance, award/dock, hints, extend a clip, …) without standing at the laptop. It also **mirrors the live round** (the map, picture, clip, and the question/answer) on the phone, so you can run map rounds without looking at the PC. Its QR sits next to the TV one in the Stream-to-TV panel; keep it to yourself (it controls the game).
- **Between-rounds recap** — when a round ends, a little fanfare plays and an animation counts each player/team up from their score at the start of the round to their score at the end (with a `+N` badge), rows **slide past each other on overtakes**, and the round's **biggest mover** gets a ★ callout — on the host and the TV. (Scoreboard numbers also count up when you award points.)

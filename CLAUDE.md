...
src/lib/model.js             pure logic: utils, ytId, mediaSource (youtube/spotify/file), mapillaryEmbedUrl, moveItem, haversineKm, fileToDataUrl, factories (incl. makeHint),
                             normalizeQuiz/normalizeGame, roundsFromImport, hintHasContent, morphValue, clipLadderActive/clipEnd, summarizeGame/aggregateLeaderboard,
...
src/components/HostRemoteView.jsx phone host controller (#/host/<code>): present/live context + sendCtrl(reveal/advance/hint/media/skipRound/award/adjust/soundOnTv/…); give/take the round's points per player; Play/Pause/Restart for clips; embeds a compact RoundBody mirror (map/image; clips show a "plays on TV/PC" placeholder — the phone never makes sound)

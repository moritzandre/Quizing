  ...
  if (!room.enabled) {
    return (
      <Button variant="outline" onClick={room.enable}>
        <Radio size={16} /> Enable phone buzzers
      </Button>
    );
  }
  ...
  return (
    <div className={`${cardCls} p-4`}>
      <div className="mb-3 flex items-center justify-between">
        ...
        <IconButton label="Turn off buzzers" onClick={room.disable} className="hover:text-red-600">
          <X size={16} />
        </IconButton>
      </div>
      ...
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1">
          ...
          {names.length ? (
            ...
            <p className="text-sm text-stone-400 dark:text-stone-500">Waiting for players to scan in…</p>
          )}
        </div>
      </div>
    </div>
